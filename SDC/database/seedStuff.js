...
let newArray = [];
let everything = (stuff) => {
  for (let i = 0; i < stuff.length; i++) {
    newArray.push(stuff[i]);
  }
}

/*
//generate the data, saving it over test.json
node seedStuff.js
//seed the data again
node index.js
...
everything(allTheData);

fs.writeFile('./test.json', JSON.stringify(newArray), 'utf8', (error) => {
  if (error) {
    console.log(error);
  } else {
    console.log('successfully wrote csv')
  }
});